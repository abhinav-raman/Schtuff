...
const Home = ({
	...
	function activeNoteBodyChangeHandler(noteBody: string, noteHtmlBody: string) {
					...
					? {
							...
							notes_list: c.notes_list.map((n) => {
								return n.created_at === activeNoteId
									? { ...n, note_body: noteBody, note_html_body: noteHtmlBody }
									: n;
							}),
					  }
					: c;
	...
	return (
		<>
			...
			<Header
				user={user}
				onMenuBtnClick={() => setIsMobileMenuVisible(!isMobileMenuVisible)}
			/>

			<main className="flex h-[calc(100vh-65px)] w-full overflow-hidden sm:h-[calc(100vh-45px)]">
				{isMobileMenuVisible && (
					<MobileMenu showMenu={isMobileMenuVisible} user={user || null} />
				)}
				{user ? (
					<>
						<LoadingIndicator />
						<div
							className={`sidebar-container flex w-1/3 snap-x backdrop-blur-sm transition-all duration-200 sm:absolute sm:z-10 sm:w-full sm:overflow-x-scroll ${
						...
						>
							...
							{activeCollectionId && (
								<NotesList
									notesList={
										data.filter((i) => i.created_at === activeCollectionId)[0]
											?.notes_list || ([] as NoteType[])
									}
						...
						{activeNoteId && (
							<NoteEditor
								activeNote={
									data
										.filter((i) => i.created_at === activeCollectionId)?.[0]
										?.notes_list.filter(
											(j) => j.created_at === activeNoteId
										)[0] || ({} as NoteType)
								}
								onActiveNoteBodyChange={activeNoteBodyChangeHandler}
							/>
						)}
					</>
				) : (
